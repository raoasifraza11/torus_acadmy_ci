...
class Auth
{
    ...
    */
    ...
    protected function init()
    {
        if ($this->CI->session->has_userdata("userID") && $this->CI->session->loginStatus) {
          ...
          //  $this->userName = $this->CI->session->userName;
            $this->userFirstName = $this->CI->session->userFirstName;
    ...
    protected function credentials($email, $password)
    {
        ...
        $user = $this->CI->db->get_where("users", array("email" => $email, "is_verified" => 1,'password'=>$password))->row();
...
//            "roles" => $this->userWiseRoles(),
      ...
      //return redirect(base_url("hostprofile"));
        return true;
    }
     ...
     * @return string
     */
    public function userFirsName()
    {
        return $this->CI->session->userdata('userfirstname');
    }
...
//    }
    ...
    public function userIsHost()
    {
        ...
        }, $this->CI->db->get_where("users", array("id" => $this->userID()))->result_array());
    }
